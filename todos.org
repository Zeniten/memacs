* TODO Set up =rassumfrassum=
:PROPERTIES:
:ADDED:    <2026-03-04 Wed>
:END:

https://github.com/joaotavora/rassumfrassum

Oh, this might got well with =mason.el=!

* TODO Check out =mason.el=
:PROPERTIES:
:ADDED:    <2026-02-28 Sat>
:END:

https://github.com/deirn/mason.el

A package manager external tools, like LSP servers, etc.

* TODO Fold code like you fold org-mode headings?
:PROPERTIES:
:ADDED:    <2026-01-11 Sun>
:END:

* TODO Does =eglot-booster= speed up Eglot in complex Clojure repos?
:PROPERTIES:
:ADDED:    <2026-01-10 Sat>
:END:

https://github.com/jdtsmith/eglot-booster

* TODO Can =consult-imenu= categorize in Clojure similar to Emacs Lisp?
:PROPERTIES:
:ADDED:    <2026-01-08 Thu>
:END:

* TODO Evaluate =helpful=, an alternative to built-in help
:PROPERTIES:
:ADDED:    <2026-01-07 Wed>
:END:

https://github.com/Wilfred/helpful

* TODO Check out encryption library =age=
:PROPERTIES:
:ADDED:    <2026-01-04 Sun>
:END:

https://github.com/anticomputer/age.el
https://edmundmiller.dev/posts/org-journal-age

* TODO Check out =significant-others=
:PROPERTIES:
:ADDED:    <2025-10-25 Sat>
:END:

https://github.com/ovistoica/significant-other.el

* TODO Check out =paw=
:PROPERTIES:
:ADDED:    <2025-04-01 Tue>
:END:

https://github.com/chenyanming/paw

* TODO Consider using =whitespace-cleanup-mode=
:PROPERTIES:
:ADDED:    <2025-03-30 Sun>
:END:

https://github.com/purcell/whitespace-cleanup-mode

There's also the new package =stripspace.el=:
https://github.com/jamescherti/stripspace.el

* TODO Check out =prodigy=
:PROPERTIES:
:ADDED:    <2025-03-30 Sun>
:END:

https://github.com/rejeep/prodigy.el

* TODO Check out =org-ql= and =org-rifle=
:PROPERTIES:
:ADDED:    <2025-03-30 Sun>
:END:

- https://github.com/alphapapa/org-ql
- https://github.com/alphapapa/org-rifle

* TODO Add =magit-todos=
:PROPERTIES:
:ADDED:    <2025-03-30 Sun>
:END:

https://github.com/alphapapa/magit-todos

* TODO Do I need to remove anything regarding =which-key= now that it's built-in?
* TODO Set up jet.el
:PROPERTIES:
:ADDED:    <2024-12-09 Mon>
:END:

https://github.com/ericdallo/jet.el

* TODO Consider disabling Evil in modes like Info, Calendar, etc.
:PROPERTIES:
:ADDED:    <2024-12-07 Sat>
:END:

#+begin_quote
u probably don't need Evil in Info, Dired, Calendar, and similar modes that
already use single keys for actions. Have you tried to use 'emacs mode in Info
mode, something like this:

(add-hook 'Info-mode (lambda () (setq evil-default-state 'emacs)))
#+end_quote

* TODO List keybindings for window switching under one heading in which-key
:PROPERTIES:
:ADDED:    <2024-11-29 Fri>
:END:

Like what Spacemacs does: 1..9: select window 1..9, instead of 1: window 1, 2:
window 2, etc.

* TODO Set up TypeScript/JavaScript packages

https://www.ovistoica.com/blog/2024-7-05-modern-emacs-typescript-web-tsx-config

* TODO Look into Combobulate

https://github.com/mickeynp/combobulate

* TODO Add "SPC TAB"
* TODO Add Nerd Fonts
:PROPERTIES:
:ADDED:    <2024-11-10 Sun>
:END:

https://github.com/rainstormstudio/nerd-icons.el

* DONE Store Tempel templates in Git
:PROPERTIES:
:ADDED:    <2026-01-20 Tue>
:DONE:     <2026-01-22 Thu>
:END:

* DONE Add support for YAML
:PROPERTIES:
:ADDED:    <2025-03-30 Sun>
:DONE:     <2025-11-19 Wed>
:END:

* DONE Pick a "REST client"
:PROPERTIES:
:ADDED:    <2025-03-30 Sun>
:DONE:     <2025-10-25 Sat>
:END:

- https://github.com/pashky/restclient.el

  The classic!

- https://github.com/federicotdn/verb

- https://github.com/JasZhe/hurl-mode

- https://github.com/gregsexton/httprepl.el

<2025-10-25 Sat>
Restclient is back through emacsorphanage:
https://github.com/emacsorphanage/restclient!

I'll run with this.

* DONE Add =html-to-hiccup=
:PROPERTIES:
:ADDED:    <2025-03-30 Sun>
:END:

https://github.com/plexus/html-to-hiccup

* DONE Set up gptel
<2024-12-01 Sun>
<2024-12-05 Thu>

* DONE Configure Memacs such that light/dark mode follows system's

<2024-12-01 Sun>

* DONE Add SPC keybindings for help pages

C-h f -> SPC h f
etc.

* DONE Set up TAB for formatting

<2024-12-01 Sun>
